...
**ダッシュボード**: https://corporatebond-hedgestrategy.streamlit.app/
...
- 欠損値は前値補完（ffill）
- 週次変化量は単純リターン（ドル円）または差分（金利・スプレッド）
...
**解釈**:
- **金利リスクはフルヘッジが最適**（h_ir=1.00）。理由は下記
...
### なぜ金利ヘッジは常に100%が最適なのか

金利ヘッジを外す（h_ir<1にする）と、固定金利キャリー（Swap_rate）を得る代わりに変動金利（SOFR）を放棄する。この「追加キャリー」と引き換えに負う「追加リスク」を比較すると:

| | 金利ヘッジあり (h_ir=1) | 金利ヘッジなし (h_ir=0) | 差分 |
|---|---|---|---|
| キャリー | SOFR = 3.67% | Swap_rate = 4.14% | **+0.47%** |
| 金利変動リスク（年率σ） | 0 | D × σ(ΔR_USD) = 6.5 × 0.58 = **3.76%** | +3.76% |
| **追加分のシャープレシオ** | | | **0.47 / 3.76 = 0.12** |

0.47%のリターンのために3.76%のリスクを取ることになり、追加分のシャープレシオはわずか0.12。全くペイしないため h_ir=1.00 に張り付く。

**為替と対照的な理由**: ドル円は過去平均+7.67%のリターンに対しボラ9.37%で、追加分のシャープレシオは約0.82。こちらはリスクを取る価値があるため h_fx<1（部分ヘッジ）が最適になる。

**背景**: 現在の米金利のイールドカーブがほぼフラット（Swap_rate - SOFR = 0.47%）であり、デュレーションリスクを取るインセンティブが小さい。仮にカーブが急スティープ化し Swap_rate - SOFR が2〜3%に開けば、h_ir<1 が最適になる可能性がある。
